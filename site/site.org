...
** TODO Monte Carlo Batch Normalization
:PROPERTIES:
...
posterior distributions of Bayes inference.
...
#
#   Warning: Network must have tf.keras.layers.BatchNormalization
mcbn = b4tf.models.MCBN(Sequential([Dense(50,use_bias=False,input_shape=(1,)),
                                    BatchNormalization(),
                                    Activation("relu"),
                                    Dense(50,use_bias=False),
                                    Activation("relu"),
                                    BatchNormalization(),
                                    Dense(1)]),
                        noise_variance=1.5,
...
# Compile MCBN. Allarguments are passed to tf.keras.Model.compile
...
m, cov = mcbn.predict(x,batch_size=32,n_batches=200)
#+end_src
