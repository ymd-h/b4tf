...
\[ f(\thinspace \cdot \thinspace ; W) \text{where} W_{ij}^{(l)} \sim \mathcal{N}(m_{ij}^{(l)}, v_{ij}^{(l)}) \text{.}\]
...
\[ y = \mathcal{N}(f(x;W), \gamma^{-1}) \text{where} \gamma \sim \mathrm{Gamma}(\alpha, \beta) \text{.}\]
